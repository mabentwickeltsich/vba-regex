...
Private Function DfsRunThreads( _
    ...
    Dim spDelta As Long ' 1 when we walk forwards and -1 when we walk backwards
        ...
        modifiers = ( _
                MODIFIER_I_ACTIVE And _
                (0 <> bytecode(BYTECODE_IDX_CASE_INSENSITIVE_INDICATOR)) _
            ) Or ( _
                MODIFIER_M_ACTIVE And multiline _
            )
...
ContinueLoopSuccess:
            ...
            Case REOP_CHAR
                ...
                If 0 <> (modifiers And MODIFIER_I_ACTIVE) Then
            ...
            Case REOP_DOT
                ...
                If 0 <> (modifiers And MODIFIER_S_ACTIVE) Then GoTo ContinueLoopSuccess
            ...
            Case REOP_ASSERT_START
                ...
                If 0 = (modifiers And MODIFIER_M_ACTIVE) Then GoTo ContinueLoopFail
                ...
                If (aa >= 0) And (bb >= 0) Then
                    If spDelta = 1 Then
                        ...
                        Do While off < bb
                            ...
                            If c1 <> c2 Then
                                If 0 = (modifiers And MODIFIER_I_ACTIVE) Then GoTo ContinueLoopFail
            ...
            Case REOP_CHANGE_MODIFIERS:
                ...
                ' We make use of the fact that for b1, b2, b3 in {0,1},
                '   (b1 ? b2 : b3)   is equivalent to   b3 xor [b1 and (b2 xor b3)]
                modifiers = modifiers Xor (((MODIFIER_WRITE_MASK And n) * 2) And (n Xor modifiers))
                GoTo ContinueLoopSuccess
...
Public Function srTryInitializeRegex( _
    ByRef regex As srRegexTy, _
    ...
    '   Then srInitializeRegex should make use of srTryInitializeRegex, not the other way round.
    On Error GoTo Fail
    srInitializeRegex regex, pattern, caseInsensitive
    srTryInitializeRegex = True
    Exit Function

Fail:
    srTryInitializeRegex = False
End Function

Public Sub srInitializeRegex( _
    ByRef regex As srRegexTy, _
    ByRef pattern As String, _
    Optional ByVal caseInsensitive As Boolean = False _
)
...
Public Function srTest( _
    ByRef regex As srRegexTy, ByRef str As String, _
    Optional ByVal multiline As Boolean = False, _
    Optional ByVal dotAll As Boolean = False _
) As Boolean
...
Public Function srMatch( _
    ByRef matcherState As srMatcherStateTy, ByRef regex As srRegexTy, ByRef haystack As String, _
    Optional ByVal multiline As Boolean = False, _
    Optional ByVal dotAll As Boolean = False, _
    Optional ByVal matchFrom As Long = 1 _
) As Boolean
    srMatch = DfsMatchFrom( _
...
Public Function srGetCapture(ByRef matcherState As srMatcherStateTy, ByRef haystack As String, Optional ByVal num As Long = 0) As String
    If num = 0 Then
        With matcherState.captures.entireMatch
            If .Length > 0 Then srGetCapture = Mid$(haystack, .start, .Length) Else srGetCapture = vbNullString
        End With
    ElseIf num <= matcherState.captures.nNumberedCaptures Then
        ...
        End With
    Else
        srGetCapture = vbNullString
    End If
End Function

Public Function srGetCaptureByName( _
    ByRef matcherState As srMatcherStateTy, _
    ByRef regex As srRegexTy, _
    ...
    If identifierId < 0 Then srGetCaptureByName = vbNullString: Exit Function
    srGetCaptureByName = srGetCapture(matcherState, haystack, matcherState.captures.namedCaptures(identifierId))
End Function

Public Function srMatchNext( _
    ByRef matcherState As srMatcherStateTy, ByRef regex As srRegexTy, ByRef haystack As String _
) As Boolean
    Dim r As Long
...
Public Function srReplace( _
    ...
    Dim parsedFormat As ArrayBuffer, matcherState As srMatcherStateTy, lastEndPos As Long, resultBuilder As StaticStringBuilder
    ...
    Do While srMatchNext(matcherState, regex, haystack)
    ...
    srReplace = GetStr(resultBuilder)
End Function

Public Function srMatchThenJoin( _
    ByRef regex As srRegexTy, _
    ByRef haystack As String, _
    ...
    Dim parsedFormat As ArrayBuffer, resultBuilder As StaticStringBuilder, matcherState As srMatcherStateTy
    ...
    matcherState.multiline = multiline
    matcherState.dotAll = dotAll
    If srMatchNext(matcherState, regex, haystack) Then
    ...
    srMatchThenJoin = GetStr(resultBuilder)
End Function

Public Sub srMatchThenList( _
    ...
    Dim matcherState As srMatcherStateTy
...
Public Sub srInitializeMatcherState( _
    ByRef matcherState As srMatcherStateTy, _
...
Public Sub srResetMatcherState(ByRef matcherState As srMatcherStateTy)
    matcherState.current = 0
End Sub
